...
public abstract class BTreePrinter {
	...
	public static void printNode(DLine root) {
		int maxLevel = BTreePrinter.maxLevel(root);

		printNodeInternal(Collections.singletonList(root), 1, maxLevel);
	}

	private static <T extends Comparable<?>> void printNodeInternal(List<DLine> nodes, int level, int maxLevel) {
		...
		List<DLine> newNodes = new ArrayList<DLine>();
		for (DLine node : nodes) {
			if (node != null) {
	...
	private static int maxLevel(DLine node) {
		if (node == null)
			return 0;
	...
	private static <T> boolean isAllElementsNull(List<DLine> list) {
